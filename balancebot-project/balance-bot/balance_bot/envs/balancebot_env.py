...
ForceObject = np.array([random.random(),random.random(),5])
PositionObject = np.array([random.random(),random.random(),random.random()])
...
class BalancebotEnv(gym.Env):
    ...
    def _reset(self):
        ...
        self._observation = self._compute_observation()
        p.applyExternalForce(objectUniqueId = self.botId, linkIndex = -1, forceObj = ForceObject, posObj = PositionObject, flags = p.LINK_FRAME)
